...
export default function Hash() {
    let [state, setState] = useState({ message: "", commit: "", state: State.Normal })
    ...
    let commit = () => {
        setState({ ...state, state: State.Normal, commit: createHash('sha1').update(state.message).digest('hex') })
    }

    let open = () => {
        let ok = createHash('sha1').update(state.message).digest('hex') == state.commit
